...
CSS = """
:root{--bg:#05060a;--panel:rgba(255,255,255,.035);--card:rgba(255,255,255,.045);--line:rgba(255,255,255,.09);--txt:#f2f5fa;--muted:#96a3b8;--red:#e5484d;--gold:#f5b83d;--green:#3fb970;--blue:#4c9aff;--grad:linear-gradient(90deg,#8b5cf6,#22d3ee)}
...
body::before{content:"";position:fixed;inset:0;z-index:-1;background:radial-gradient(640px 420px at 12% -5%,rgba(139,92,246,.17),transparent 60%),radial-gradient(720px 440px at 88% -8%,rgba(34,211,238,.13),transparent 60%),radial-gradient(900px 520px at 50% 112%,rgba(139,92,246,.06),transparent 60%)}
::selection{background:rgba(139,92,246,.45)}
a{color:#7dd3fc;text-decoration:none}a:hover{text-decoration:underline}
nav{position:sticky;top:0;z-index:50;backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);background:rgba(5,6,10,.74);border-bottom:1px solid var(--line)}
.navi{max-width:1100px;margin:0 auto;padding:10px 24px;display:flex;align-items:center;gap:16px}
.brand{font-weight:800;letter-spacing:-.02em;font-size:.95rem;white-space:nowrap;background:linear-gradient(90deg,#8b5cf6,#22d3ee);-webkit-background-clip:text;background-clip:text;color:transparent}
.pills{display:flex;gap:6px;overflow-x:auto;scrollbar-width:none}
.pills::-webkit-scrollbar{display:none}
.pills a{white-space:nowrap;font-size:.75rem;font-weight:600;color:var(--muted);border:1px solid var(--line);border-radius:999px;padding:4px 11px;transition:color .15s,border-color .15s,transform .15s}
.pills a:hover{color:var(--txt);border-color:rgba(139,92,246,.55);text-decoration:none;transform:translateY(-1px)}
header{padding:54px 24px 8px;max-width:1100px;margin:0 auto}
h1{font-size:clamp(2.1rem,5vw,3rem);letter-spacing:-.03em;line-height:1.08}
.gx{background:linear-gradient(90deg,#a78bfa,#22d3ee);-webkit-background-clip:text;background-clip:text;color:transparent}
.live{display:inline-flex;align-items:center;gap:7px;font-size:.72rem;color:var(--muted);border:1px solid var(--line);border-radius:999px;padding:3px 11px;vertical-align:1px}
.live i{width:7px;height:7px;border-radius:50%;background:#3fb970;box-shadow:0 0 0 0 rgba(63,185,112,.5);animation:pulse 2.2s infinite}
@keyframes pulse{70%{box-shadow:0 0 0 8px rgba(63,185,112,0)}100%{box-shadow:0 0 0 0 rgba(63,185,112,0)}}
.sub{color:var(--muted);margin-top:10px;display:flex;align-items:center;gap:8px;flex-wrap:wrap;font-size:.92rem}
.stats{display:flex;gap:12px;flex-wrap:wrap;margin-top:22px}
.stat{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:12px 18px;min-width:118px;backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px)}
.stat b{display:block;font-size:1.6rem;letter-spacing:-.02em;background:linear-gradient(90deg,#c4b5fd,#67e8f9);-webkit-background-clip:text;background-clip:text;color:transparent}
.stat span{color:var(--muted);font-size:.74rem;text-transform:uppercase;letter-spacing:.08em}
main{max-width:1100px;margin:0 auto;padding:0 24px}
section{margin-top:56px;scroll-margin-top:78px}
h2{font-size:1.42rem;letter-spacing:-.02em;display:flex;align-items:center;gap:11px}
h2::before{content:"";width:4px;height:1.02em;border-radius:4px;background:linear-gradient(180deg,#8b5cf6,#22d3ee)}
.count{background:var(--panel);border:1px solid var(--line);color:var(--muted);font-size:.76rem;border-radius:999px;padding:2px 10px;font-weight:700}
.sub2{color:var(--muted);font-size:.9rem;margin:6px 0 18px}
.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(330px,1fr));gap:16px}
.card{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:15px;display:flex;gap:14px;transition:transform .18s ease,border-color .18s ease,box-shadow .18s ease}
.card:hover{transform:translateY(-3px);border-color:rgba(139,92,246,.5);box-shadow:0 14px 34px -14px rgba(139,92,246,.3)}
.poster{position:relative;width:88px;min-height:132px;flex-shrink:0}
.poster .po{width:100%;height:100%;min-height:132px;object-fit:cover;object-position:center top;border-radius:10px;display:block;background:var(--panel)}
.ph{display:none;position:absolute;inset:0;border-radius:10px;background:linear-gradient(150deg,rgba(139,92,246,.22),rgba(34,211,238,.1));align-items:center;justify-content:center;font-weight:800;font-size:1.7rem;color:rgba(255,255,255,.5)}
.cbody{flex:1;min-width:0;display:flex;flex-direction:column;gap:7px}
.t{font-weight:700;font-size:1.03rem;color:var(--txt);overflow-wrap:break-word;letter-spacing:-.01em}
a.t{color:var(--txt)}a.t:hover{color:#a78bfa;text-decoration:none}
.plat{display:inline-block;align-self:flex-start;font-size:.7rem;font-weight:700;letter-spacing:.05em;text-transform:uppercase;color:#67e8f9;background:rgba(34,211,238,.07);border:1px solid rgba(34,211,238,.28);border-radius:7px;padding:2px 8px}
.cbody p{color:var(--muted);font-size:.86rem}
.meta{display:flex;align-items:center;gap:8px;flex-wrap:wrap;color:var(--muted);font-size:.73rem}
.chip{background:var(--panel);border:1px solid var(--line);border-radius:999px;padding:1px 8px}
.ggrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(245px,1fr));gap:12px}
.gcard{background:var(--card);border:1px solid var(--line);border-radius:14px;padding:12px 14px;transition:border-color .18s ease,transform .18s ease}
.gcard:hover{border-color:rgba(34,211,238,.45);transform:translateY(-2px)}
.gname{font-size:.7rem;letter-spacing:.14em;text-transform:uppercase;color:#67e8f9}
.gtitle{margin:3px 0;font-weight:650;font-size:.95rem}
.gtitle a{color:var(--txt)}
.gmeta{font-size:.78rem;color:var(--muted)}
.gwhy{font-size:.79rem;color:var(--muted);margin-top:4px}
.gnext{font-size:.73rem;color:#a78bfa;margin-top:7px}
...
.bar{flex:1;height:6px;background:rgba(255,255,255,.06);border-radius:999px;overflow:hidden}
.bar i{display:block;height:100%;border-radius:999px}
.bar i.hi{background:linear-gradient(90deg,#8b5cf6,#22d3ee)}
.bar i.mid{background:linear-gradient(90deg,#f5b83d,#f97316)}
.bar i.low{background:linear-gradient(90deg,#4c9aff,#22d3ee)}
#alerts{background:linear-gradient(180deg,rgba(229,72,77,.09),rgba(229,72,77,.02));border:1px solid rgba(229,72,77,.3);border-radius:18px;padding:22px 24px}
...
.empty{color:var(--muted);font-style:italic;font-size:.9rem;background:var(--panel);border:1px dashed var(--line2,var(--line));border-radius:12px;padding:12px 14px}
table{width:100%;border-collapse:collapse;background:rgba(255,255,255,.03);border:1px solid var(--line);border-radius:14px;overflow:hidden;font-size:.87rem}
th,td{text-align:left;padding:10px 13px;border-bottom:1px solid var(--line);vertical-align:top}
th{background:rgba(255,255,255,.04);color:var(--muted);font-size:.72rem;text-transform:uppercase;letter-spacing:.07em}
tr:hover td{background:rgba(139,92,246,.05)}
tr:last-child td{border-bottom:none}
code{background:rgba(139,92,246,.1);border:1px solid rgba(139,92,246,.3);border-radius:7px;padding:1px 7px;font-size:.82rem;color:#c4b5fd}
.muted{color:var(--muted)}
.log-wrap{overflow-x:auto;border-radius:14px}
footer{max-width:1100px;margin:56px auto 0;padding:18px 24px 0;color:var(--muted);font-size:.8rem;display:flex;gap:16px;flex-wrap:wrap;border-top:1px solid var(--line)}
footer a{color:var(--muted)}footer a:hover{color:var(--txt)}
@media(max-width:640px){h1{font-size:1.9rem}.grid{grid-template-columns:1fr}header{padding:34px 16px 8px}main{padding:0 16px}.poster{width:72px;min-height:110px}.poster .po{min-height:110px}.navi{padding:9px 14px;gap:10px}h2{font-size:1.22rem}}
...
<body>
<nav><div class="navi"><span class="brand">▶ Streaming Watchlist</span><div class="pills">
<a href="#alerts">Alerts</a><a href="#picks">Genre Picks</a><a href="#mustWatch">Must Watch</a><a href="#highlyRecommended">Recommended</a><a href="#hiddenGems">Hidden Gems</a><a href="#indianPicks">Indian</a><a href="#youtubePicks">YouTube</a><a href="#futurePicks">Future &amp; Learn</a><a href="#aiPicks">AI</a><a href="#sportsPicks">Sports</a><a href="#gadgetPicks">Gadgets</a><a href="#greenPicks">Green Energy</a><a href="#businessPicks">Business</a><a href="#moneyPicks">Money</a><a href="#upcoming">Upcoming</a><a href="#log">Change Log</a>
</div></div></nav>
<header>
  <h1>🎬 <span class="gx">Streaming Watchlist</span></h1>
  <p class="sub"><span>India · legitimate services only</span><span class="live"><i></i>auto-updated hourly</span><span>last built {esc(now)}</span><span>{posters_on_disk} posters cached</span></p>
  <div class="stats">
